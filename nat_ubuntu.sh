...
function remote_daemon1 {
	...
	screen -dmS $daemon1 /usr/bin/mono $c9ssh_root"nat123linux.sh" autologin  stone55 ft123456!
}

function remote_daemon2 {
	kill_daemon2
	echo "start remote_daemon2"
	screen -dmS $daemon2 /usr/bin/mono $c9ssh_root"nat123linux.sh" service
}
...
kill_daemon
screen -dm -S $daemon /bin/bash $remote_sh
